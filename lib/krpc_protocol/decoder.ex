defmodule KRPCProtocol.Decoder do
  ...
  def decode(%{
        ...
        "t" => tid,
        "y" => "q",
        "a" => %{"id" => node_id, "info_hash" => infohash, "port" => port, "token" => token}
      }) do
  ...
  def decode(%{"y" => "r", "t" => tid, "r" => %{"id" => id, "token" => t, "values" => values}}) do
    {:get_peer_reply,
     %{tid: tid, node_id: id, token: t, values: extract_values(values), nodes: nil}}
  end

  def decode(%{"y" => "r", "t" => tid, "r" => %{"id" => id, "token" => t, "nodes" => nodes}}) do
    {:get_peer_reply,
     %{tid: tid, node_id: id, token: t, values: nil, nodes: extract_nodes(nodes)}}
  end

  def decode(%{"y" => "r", "t" => tid, "r" => %{"id" => id, "token" => t, "nodes6" => nodes}}) do
    {:get_peer_reply,
     %{tid: tid, node_id: id, token: t, values: nil, nodes: extract_nodes6(nodes)}}
  end
  ...
  defp extract_nodes(
         <<id::binary-size(20), addr::binary-size(6), tail::binary>>,
         result
       ) do
  ...
  defp extract_nodes6(
         <<id::binary-size(20), addr::binary-size(18), tail::binary>>,
         result
       ) do
